...
class NBClassifier:
    ...
    def getFeatures(self, email, number):
    ...
    def train(self, lineClasses):
        ...
        for index, row in classFeatureSums.iterrows():
            ...
            for column in classFeatureSums:
                ...
                if not column == 'prevLineClass': # don't add inverse for non binary columns
    ...
    def makePrediction(self, email, lineNo, prevPrediction):
        ...
        testLine = pd.DataFrame.from_records([self.getFeatures(email, lineNo)])
